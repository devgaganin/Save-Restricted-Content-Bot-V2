...
async def get_msg(userbot, sender, edit_id, msg_link, i, message):
    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
        ...
        if chat in saved_channel_ids:
            await app.edit_message_text(message.chat.id, edit_id, "Sorry! dude 😎 This channel is protected 🔐 by **__ilaps__**")
            return
...
def get_user_rename_preference(user_id):
    # Retrieve the user's custom renaming tag if set, or default to 'ilaps'
    return user_rename_preferences.get(str(user_id), 'ilaps')
...
async def settings_command(event):
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/ilaps")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        ...
        await event.edit("Choose your preferred upload method:\n\n__**Note:** **SpyLib ⚡**, built on Telethon(base), by ilaps still in beta.__", buttons=buttons)
