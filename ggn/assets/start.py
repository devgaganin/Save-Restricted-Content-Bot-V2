...
async def start(event):
    ...
    collection.update_one({"user_id": user_id}, {"$set": {"user_id": user_id}}, upsert=True)
    buttons = [
        [Button.url("Join Channel", url="https://t.me/+Se8CtSz3_GFiMDU0")],
...
S = "/start"
START_PIC = "https://tecolotito.elsiglocoahuila.mx/i/2023/12/2131463.jpeg"
...
PRE_TEXT = """💰 **Premium Price**: Starting from INR 39, contact **@xadarsh_x** (terms and conditions apply).
...
async def plan_command(event):
    ...
    buttons = [
        [Button.url("Contact for Purchase", url="https://t.me/xadarsh")]
    ]
...
async def term_command(event):
    # Creating inline keyboard with buttons
    buttons = [
        [Button.url("Query?", url="https://t.me/xadrsh_x"),
...
REPO_URL = "https://tecolotito.elsiglocoahuila.mx/i/2023/12/2131463.jpeg"
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [Adarsh](https://t.me/xadarsh_x)**__"  # Set caption to the title of the video
